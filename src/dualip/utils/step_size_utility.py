...
def norm_of_difference(x: np.ndarray, y: np.ndarray) -> float:
    """
...
def update_dual_gradient_history(
    ...
    if len(grad_history) == max_history_length:
        ...
        dual_history.pop(0)
        grad_history.pop(0)
    grad_history.append(np.array(gradient))
    dual_history.append(np.array(dual_val))
